        ...
        if(len(df_search) > 0):
            ...
            with col1:
                ...
                with st.form("reevaluate_risk_form"):
                    ...
                    # number_floors = st.text_input("Number of floors in building: ", str(df_search["number_floors"][0]), key='number_floors')
            ...
            with col2:
                with st.container():
                    ...
                    if st.session_state['reevaluate_building_risk']:
                        ...
                        age_building = np.log(int(str(age_building).replace("0", "1")))
                        area_sq_ft = np.log(int(str(area_sq_ft).replace("0", "1")))
                        height_ft = np.log(int(str(height_ft).replace("0", "1")))
                        ...
                        df_predict = [[age_building,area_sq_ft,height_ft,has_superstructure_adobe_mud,has_superstructure_mud_mortar_stone,has_superstructure_stone_flag,
                                       ...
                                       has_superstructure_other,district_id_weighted_mean,mun_id_weighted_mean,ward_id_weighted_mean,land_surface_Moderate_slope,land_surface_Steep_slope,
                        ...
                        # Position and Plan configuration
                        # plan_index = df_plan_configuration[(df_plan_configuration['plan_configuration']==st.session_state.plan_configuration)].index[0].item()
                        # plan_configuration = st.selectbox("Plan Configuration of Building: ", df_plan_configuration, index=plan_index)
                        ...
                        # Number of floors and Height
                        # number_floors = st.text_input("Number of floors in building: ", st.session_state.number_floors)
                        ...
                        st.write(current_damage_grade)
                        if (dmg_grade > int(current_damage_grade)):
                            st.markdown("##### Predicted Damage Grade: **" + str(dmg_grade) + "**" + " :arrow_up:")
                        if (dmg_grade < int(current_damage_grade)):
                            st.markdown("##### Predicted Damage Grade: **" + str(dmg_grade) + "**" + " :arrow_down:")
                        if (dmg_grade == int(current_damage_grade)):
                            st.markdown("##### Predicted Damage Grade: **" + str(dmg_grade) + "**")
                        # Up or down, comments of damage grade
                        if (dmg_grade == 0):
                            st.markdown("###### Minor repair required")
                        if (dmg_grade == 1):
                            st.markdown("###### Major repair required")
                        if (dmg_grade == 2):
                            st.markdown("Total reconstruction required")
